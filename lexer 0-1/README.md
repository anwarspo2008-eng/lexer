...
This is meant as a clear, simple example of how lexers work, not a
production-grade C compiler front end. A few things it deliberately
does **not** handle:

- **Preprocessor directives** like `#include` or `#define` — the `#`
  itself comes back as `TOKEN_UNKNOWN`; the rest of the line is
  tokenized normally as separate tokens.
- **Wide/unicode characters, digraphs/trigraphs, or exotic numeric
  literals** (hex `0x1F`, octal `010`, suffixes like `10UL`) are not
  specially recognized — hex numbers, for instance, will be split
  into a `0` and an identifier-like piece.
- It only reports tokens; it does not build a syntax tree or check
  whether the code is grammatically valid C.

These would all be reasonable next steps if you want to extend it.
...
- Add hex/octal/binary number support in `read_number()`.
- Add more keywords to the `KEYWORDS` array in `lexer.c`.
- Add a `TOKEN_PREPROCESSOR` type to handle `#include`, `#define`, etc.
  as single tokens.
- Feed the token stream into a parser to build an abstract syntax
  tree (the next stage after lexing).
